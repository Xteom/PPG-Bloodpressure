...
def train_approximate_network():
    ...
    for foldname in range(10):
        ...
        print('----------------')
        print(f'Training Fold {foldname}')
        print('----------------')
                                                                                            # loading training data
        dt = pickle.load(open(os.path.join('data',f'train{foldname}.p'),'rb'))
        X_train = dt['X_train']
        Y_train = dt['Y_train']
                                                                                            # loading validation data
        dt = pickle.load(open(os.path.join('data',f'val{foldname}.p'),'rb'))
        X_val = dt['X_val']
        Y_val = dt['Y_val']

                                                                                            # loading metadata
        dt = pickle.load(open(os.path.join('data',f'meta{foldname}.p'),'rb'))
        max_ppg = dt['max_ppg']
        min_ppg = dt['min_ppg']
        max_abp = dt['max_abp']
        min_abp = dt['min_abp']
        ...
        checkpoint1_ = ModelCheckpoint(os.path.join('models',f'{mdlName1}_model1_fold{foldname}.h5'), verbose=1, monitor='val_out_loss',save_best_only=True, mode='auto')  
        ...
        pickle.dump(history1, open(f'History/{mdlName1}_model1_fold{foldname}.p','wb'))    # save training history
...
def train_refinement_network():
    ...
    for foldname in range(10):
        ...
        mdl1.load_weights(os.path.join('models',f'{mdlName1}_model1_fold{foldname}.h5'))   # load weights
        ...
        checkpoint2_ = ModelCheckpoint(os.path.join('models',f'{mdlName2}_model2_fold{foldname}.h5'), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')  
        ...
        pickle.dump(history2, open(f'History/{mdlName2}_model2_fold{foldname}.p','wb'))    # save training history
